...
class FoundationTrainer:
    ...
    def add_as_built_data(self, plan_id: str, plan_type: str, as_built_image: np.ndarray,
                         final_elements: List[Dict[str, Any]], construction_notes: str,
                         approval_date: datetime, project_info: Dict[str, Any]):
    ...
    def add_review_milestone(self, plan_id: str, milestone: str, reviewer_comments: List[str],
                           approved_elements: List[Dict[str, Any]], 
                           rejected_elements: List[Dict[str, Any]],
                           compliance_score: float, review_date: datetime):
    ...
    def extract_training_features(self):
        ...
        print(f"✅ Extracted features from {len(self.training_features)} training examples")
    ...
    def train_foundation_models(self, min_examples: int = 20):
        ...
        if len(self.training_features) < min_examples:
            print(f"❌ Need at least {min_examples} examples, have {len(self.training_features)}")
            return
        
        print(f"🤖 Training foundation models on {len(self.training_features)} examples...")
        ...
        model_types = {
            'foundation_random_forest': RandomForestClassifier(n_estimators=200, random_state=42),
    ...
    def _extract_milestone_features(self, milestone: ReviewMilestone) -> np.ndarray:
        ...
        # Comment features
        features.extend(self._extract_text_features(' '.join(milestone.reviewer_comments)))
    ...
    def _extract_milestone_labels(self, milestone: ReviewMilestone) -> np.ndarray:
        ...
        # Approval ratio
        total_elements = len(milestone.approved_elements) + len(milestone.rejected_elements)
        approval_ratio = len(milestone.approved_elements) / total_elements if total_elements > 0 else 1.0
    ...
    def _extract_plan_features(self, plan_image: np.ndarray, 
                             detected_elements: List[Any]) -> np.ndarray:
    ...
    def _save_as_built_data(self, as_built: AsBuiltData):
        ...
        as_built_dict = {
            ...
            'plan_type': as_built.plan_type,
            'as_built_image': cv2.imencode('.png', as_built.as_built_image)[1].tobytes(),
    ...
    def _save_review_milestone(self, milestone: ReviewMilestone):
        ...
        data_file = self.data_dir / f"milestone_{milestone.plan_id}_{milestone.milestone}.pkl"
    ...
    def _save_training_statistics(self, X_train: np.ndarray, X_test: np.ndarray,
