...
try:
    ...
    print(
        "Warning: ML dependencies not installed. Install with: pip install torch scikit-learn joblib"
    )
...
class AdaptiveReviewer:
    ...
    def review_plan(
        self, plan_image: np.ndarray, detected_elements: List[Any], plan_type: str
    ) -> Dict[str, Any]:
        """Review plan with adaptive learning capabilities."""

        # Extract features
        features = self.feature_extractor.extract_features(
            plan_image, detected_elements
        )
    ...
    def train_models(self, min_examples: int = 10):
        ...
        for model_name, model in model_types.items():
            ...
            print(
                f"{model_name} - Train accuracy: {train_score:.3f}, Test accuracy: {test_score:.3f}"
            )
    ...
    def _create_training_example(self, feedback: ReviewFeedback):
        ...
        predicted_labels = self._extract_labels_from_prediction(
    ...
    def _extract_labels_from_corrections(
    ...
    def calculate_feature_importance(
        ...
        if not hasattr(model, "feature_importances_"):
            raise ValueError(
                f"Model '{model_name}' is not a tree-based model. Tree-based models include RandomForest, GradientBoosting, etc."
            )
        ...
        logger.info(
            f"Calculated feature importance for model '{model_name}' with {len(feature_names)} features"
        )
        logger.debug(f"Top 5 features: {list(sorted_importance.items())[:5]}")

        return sorted_importance

    def get_feature_importance_json(
        self, model_name: str, feature_names: List[str]
    ) -> str:
        """
