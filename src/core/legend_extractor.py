...
def extract_symbols_from_legend(pdf_path, page_number, output_dir="templates/existing"):
    ...
    print("🖱️ Select each symbol from the legend. Drag a rectangle, release mouse. Press ESC when done.")
    while True:
        cv2.imshow("Legend Sheet", img)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Extract legend symbols from a PDF page.")
    parser.add_argument("pdf_path", help="Path to the PDF file")
    parser.add_argument("--page", type=int, default=0, help="Page number (0-based index)")
    parser.add_argument("--output_dir", default="templates/existing", help="Directory to save extracted symbols")
