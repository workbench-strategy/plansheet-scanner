...
class ComplianceReviewer:
    ...
    def review_code(self, code: str, frameworks: List[str] = None) -> ReviewPerspective:
        ...
        for framework in frameworks:
            if framework in self.compliance_frameworks:
                ...
                compliance_status[framework] = len([f for f in framework_findings if f['severity'] == 'critical']) == 0
    ...
    def _check_framework_compliance(self, code: str, framework: str) -> List[Dict[str, Any]]:
...
class BusinessReviewer:
    ...
    def _analyze_maintainability(self, code: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if complexity > self.business_metrics['maintainability']['complexity_threshold']:
                findings.append({
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    lines = len(node.body)
                    if lines > self.business_metrics['maintainability']['function_length_threshold']:
                        findings.append({
                            'type': 'maintainability_issue',
    ...
    def _calculate_business_impact(self, findings: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Calculate business impact of findings."""
        return {
            'maintainability_score': max(0, 100 - len([f for f in findings if f['category'] == 'maintainability_issue']) * 10),
            'scalability_score': max(0, 100 - len([f for f in findings if f['category'] == 'scalability_issue']) * 10),
...
class InterdisciplinaryReviewer:
    ...
    def perform_review(self, file_path: str, frameworks: List[str] = None, domain: str = 'general') -> InterdisciplinaryReview:
        ...
        # Calculate business impact
        business_impact = business_perspective.findings[0] if business_perspective.findings else {}
    ...
    def _generate_recommendations(self, perspectives: Dict[str, ReviewPerspective]) -> List[str]:
        ...
        for finding_data in all_findings[:10]:  # Top 10 recommendations
            ...
            if 'recommendation' in finding:
                recommendations.append(f"[{perspective.upper()}] {finding['recommendation']}")
