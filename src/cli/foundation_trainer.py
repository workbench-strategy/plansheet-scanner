...
def export_data_command(args):
    ...
    if args.format == 'json':
        if as_built_data:
            ...
            print(f"✅ Exported {len(as_built_data)} as-built records to {args.output}_as_built.json")
        
        if milestone_data:
            with open(f"{args.output}_milestones.json", 'w') as f:
                json.dump(milestone_data, f, indent=2, default=str)
            print(f"✅ Exported {len(milestone_data)} milestone records to {args.output}_milestones.json")
        ...
        if as_built_data:
            ...
            print(f"✅ Exported {len(as_built_data)} as-built records to {args.output}_as_built.csv")
        
        if milestone_data:
            df_milestones = pd.DataFrame(milestone_data)
            df_milestones.to_csv(f"{args.output}_milestones.csv", index=False)
            print(f"✅ Exported {len(milestone_data)} milestone records to {args.output}_milestones.csv")
...
Examples:
  ...
  python foundation_trainer.py batch-import /path/to/training/data
    ...
    parser.add_argument('--data-dir', default='training_data', help='Directory for training data')
    parser.add_argument('--model-dir', default='models', help='Directory for trained models')
    
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    
    # Add as-built command
    as_built_parser = subparsers.add_parser('add-as-built', help='Add as-built data for training')
    as_built_parser.add_argument('plan_id', help='Unique plan identifier')
    as_built_parser.add_argument('plan_type', help='Type of plan (traffic_signal, its, mutcd_signing)')
    ...
    as_built_parser.add_argument('--project-info', help='Path to project info JSON file')
    ...
    milestone_parser = subparsers.add_parser('add-milestone', help='Add review milestone data')
    milestone_parser.add_argument('plan_id', help='Unique plan identifier')
    milestone_parser.add_argument('milestone', choices=['preliminary', 'final', 'construction', 'as_built'], help='Review milestone type')
    milestone_parser.add_argument('--approved-elements', help='Path to approved elements JSON file')
    milestone_parser.add_argument('--rejected-elements', help='Path to rejected elements JSON file')
    milestone_parser.add_argument('--comments', help='Path to reviewer comments JSON file')
    milestone_parser.add_argument('--compliance-score', type=float, default=0.5, help='Compliance score (0-1)')
    ...
    train_parser.add_argument('--min-examples', type=int, default=20, help='Minimum examples required for training')
    train_parser.set_defaults(func=train_models_command)
    
    # Show statistics command
    stats_parser = subparsers.add_parser('show-statistics', help='Show training statistics')
    stats_parser.set_defaults(func=show_statistics_command)
    
    # Predict command
    predict_parser = subparsers.add_parser('predict', help='Make prediction with foundation models')
    predict_parser.add_argument('plan_image', help='Path to plan image')
    predict_parser.add_argument('--detected-elements', help='Path to detected elements JSON file')
    ...
    import_parser = subparsers.add_parser('batch-import', help='Batch import from directory')
    import_parser.add_argument('data_directory', help='Directory containing training data files')
