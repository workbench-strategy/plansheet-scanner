...
def import_feedback_command(args):
    ...
    for feedback in feedback_data:
        try:
            ...
            print(
                f"⚠️ Error importing feedback {feedback.get('plan_id', 'unknown')}: {e}"
            )
...
Examples:
  ...
  python learning_manager.py reset-models --feedback --force
    ...
    parser.add_argument(
        "--model-dir", default="models", help="Directory for models and feedback"
    )
    ...
    collect_parser = subparsers.add_parser(
        "collect-feedback", help="Collect feedback from human review"
    )
    ...
    collect_parser.add_argument(
        "--confidence", type=float, default=0.5, help="Confidence score (0-1)"
    )
    collect_parser.add_argument(
        "--review-time", type=float, default=0.0, help="Review time in seconds"
    )
    collect_parser.add_argument("--notes", default="", help="Additional notes")
    collect_parser.add_argument(
        "--original-prediction", help="Path to original prediction JSON file"
    )
    collect_parser.add_argument(
        "--human-corrections", help="Path to human corrections JSON file"
    )
    collect_parser.set_defaults(func=collect_feedback_command)

    # Train models command
    train_parser = subparsers.add_parser(
        "train-models", help="Train models on accumulated feedback"
    )
    ...
    stats_parser = subparsers.add_parser(
        "show-statistics", help="Show learning system statistics"
    )
    stats_parser.set_defaults(func=show_statistics_command)

    # Export feedback command
    export_parser = subparsers.add_parser(
        "export-feedback", help="Export feedback data"
    )
    ...
    import_parser = subparsers.add_parser(
        "import-feedback", help="Import feedback data"
    )
    ...
    reset_parser = subparsers.add_parser(
        "reset-models", help="Reset models and feedback data"
    )
    reset_parser.add_argument(
        "--feedback", action="store_true", help="Also delete feedback data"
    )
